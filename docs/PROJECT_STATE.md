...
- [ ] PR-4 Experience Runtime Review Gate：PR-3 RunStore Identity Migration Implementation + Closure Review 已完成；下一步如需进入 PR-4，必须先 review Experience Runtime contract（Candidate selection / Projection / Experience reasoning），**当前不写 PR-4 实现、不接 Experience Runtime / Memory / ArtifactReader / E2E**。
