...
状态：**提议（Proposed）** —— 待 Review Gate 批准。**本 ADR 与配套 [M3.6 Data Provider Layer 设计说明](../design/m3.6-data-provider-layer-design.md) 均 doc-only：不写实现、不建 provider 目录、不接 iFinD/Wind/akshare、不改 schema、不申请任何 token；实现前继续 Review Gate（下一阶段 = M3.6.0 Provider Contract 骨架，仍需批准后开工）。**
日期：2026-07-01
目标版本：v0.3.0
...
- **M3.2 的 `PublicMarketDataProvider` + `acquisition.py` + mapper/fact_mapper 链保留为 legacy 已验证路径**，标注渐迁，不在本里程碑推倒。理由：它已跑通 EastMoney/CNInfo/Tushare + 11 套测试，是「让现实撞模型」的既有成果；强行改造回归面过大。渐迁方向：未来 EastMoney/CNInfo 各实现一个 `ObservationProvider` adapter，产出 `ObservationDraft` 走新 ingestion pipeline，逐步让 `MarketFact`-投影路径与 `Observation`-append 路径并存直至前者可退役（另经 Review）。
...
Provider Adapter（每源一个）
  fetch_observations(query) → tuple[ObservationDraft, ...]
       ...
       —— provider 只负责「取数 + 把源结构映射成语义草稿」，不算身份、不写库
        │
        ▼
Observation Ingestion Pipeline（共享，唯一一处；M3.6.2）
...
### D8 — 子阶段序列：先 Mock，再 iFinD

```
M3.6.0 Provider Contract        ObservationProvider / DataQuery / ObservationDraft 契约骨架（doc→code 待 Review）
   ↓
M3.6.1 MockProvider             确定性 mock adapter，产 ObservationDraft
   ↓
M3.6.2 Observation ingestion    共享 pipeline：定身份 + 幂等写 spine（打通 Mock → Observation → Evolution → Context 全链路）
   ↓
M3.6.3 iFinD Adapter            第一个真实优先 provider（client + mapper + config），可替换
   ↓
M3.6.4 akshare Adapter          免费默认源 adapter
   ↓
...
- ❌ 任何实现代码（本 ADR + 设计说明均 doc-only；M3.6.0 骨架亦需下一轮 Review 批准后才写）
...
- iFinD / akshare adapter 的具体实现与 SDK 接入（M3.6.3 / M3.6.4，各经 Review）。
- 共享 ingestion pipeline 的 `content_hash` / `logical_key` **具体算法**（M3.6.2 落地；本 ADR 只冻结「在 pipeline 一处、确定性、幂等」原则）。
...
- 为 M3.6 实现（M3.6.0→M3.6.4，逐段 Review）冻结 provider 契约、归一化边界、Boundary 6 条、配置策略、Observation-iFinD 关系。
