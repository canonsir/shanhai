...
补充约束（EvoMap 复盘冻结，2026-06-23，仅声明）：

- **`ExperienceArtifact` 是认知能力单元，不是「更好的 lesson」**：EvoMap 的核心启发不是 Memory，而是 `Experience = 可传播、可评价、可演化的能力单元`。Artifact 须具备 `identity / capability / context / evidence / applicability / evaluation / lineage / evolution history`，而**不是** `title / content / embedding`。否则会退化为「失败记录 → 总结经验 → 存知识库」的传统 RAG，而非 Agent 自我进化系统。
- **`ExperienceArtifact ≠ Knowledge Document`**：四类职责不同——`ExperienceEvent`=事实记录、`ExperienceCandidate`=待验证经验假设、`ExperienceArtifact`=已验证可复用能力、`Knowledge Document`=外部知识信息。关系链：`External Knowledge → Agent → ExperienceEvent → ExperienceCandidate →(validation)→ ExperienceArtifact →(projection)→ WeKnora / llm-wiki`。**不得让 WeKnora / llm-wiki 成为 Experience 的存储**；它们是 Artifact 的下游知识投影层。
- **本 ADR 范围边界（ADR 0015 vs ADR 0016）**：ADR 0015 解决「**如何让事实进入系统**」（事实链 + Outcome 回填）；`ExperienceCandidate / ExperienceArtifact / Validation / Promotion / Lineage / Evolution` 的完整模型属「**如何让事实成长为能力**」，**另立 [ADR 0016（Experience Evolution Layer Architecture，Proposed）](0016-Experience-Evolution-Layer-Architecture.md)**，不并入本 ADR。
